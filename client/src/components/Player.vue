<template>
  ...
  >
    ...
    <div id="player-profile-section" class="flex flex-col items-center mb-2">
      ...
      <div id="player-name-text" class="font-bold text-base truncate w-full text-center">
        {{ playerName }}
      </div>
    </div>

    <!-- Cards -->
    <div id="player-cards-container" class="flex -space-x-4 mb-2 h-16 items-center justify-center">
      ...
      <template v-else>
        <!-- Show Backs -->
        <CardBack :id="'player-card-' + playerName + '-back-1'" :size="'small'" />
        <CardBack :id="'player-card-' + playerName + '-back-2'" :size="'small'" class="-ml-4" />
      </template>
    </div>

    <!-- Chips & Action -->
    <div id="player-chips-action-section" class="w-full text-center">
      <div id="player-chips-display" class="text-yellow-400 font-mono text-base">${{ playerChips }}</div>
      <div v-if="playerAction" id="player-action-display" class="text-sm text-gray-200 mt-1 italic font-medium">
        {{ playerAction }}
      </div>
    </div>
  </div>
