<template>
  <div
    id="poker-action-hud"
    class="fixed bottom-0 left-0 right-0 z-50 p-2 md:p-3 pointer-events-none"
  >
    <div
      ...
      :class="
        isMyTurn
          ? 'border-yellow-500/50 bg-gray-800/95 shadow-[0_-10px_40px_rgba(234,179,8,0.2)]'
          : 'border-white/10'
      "
    >
      <div
        id="hud-user-info"
        class="flex items-center gap-4 md:border-r border-white/10 md:pr-5"
      >
        <div class="flex flex-col">
          <span
            id="label-stack"
            class="text-[7px] font-black text-gray-500 uppercase tracking-widest"
            >Your Chips</span
          >
          <span
            id="display-balance"
            class="text-base md:text-lg font-mono font-black text-white leading-none"
            >${{ balance }}</span
          >
        </div>
      </div>
      ...
      <div
        ...
        class="flex gap-1 items-center px-2 md:px-4 md:border-r border-white/10"
      >
        ...
        <template v-else>
          <div class="flex -space-x-2 opacity-20">
            <div
              class="w-6 h-10 md:w-8 md:h-12 bg-gray-800 border border-white/10 rounded-md"
            ></div>
            <div
              class="w-6 h-10 md:w-8 md:h-12 bg-gray-800 border border-white/10 rounded-md"
            ></div>
      ...
      <div
        ...
        class="flex items-center gap-2 md:gap-4 px-2 flex-grow justify-center min-w-full md:min-w-0 order-first md:order-none mb-2 md:mb-0"
      >
        <div class="flex flex-col items-end min-w-[50px] md:min-w-[60px]">
          <span
            id="label-raise-to"
            class="text-[7px] font-black text-yellow-500 uppercase"
            >Raise To</span
          >
          <span
            id="display-raise-amount"
            class="text-sm md:text-md font-mono font-bold text-white leading-none"
            >${{ betAmount }}</span
          >
        </div>
        ...
        <div class="flex flex-col items-center gap-1">
          ...
          <div class="flex justify-between w-full px-1">
            <span class="text-[6px] text-gray-500 font-bold font-mono"
              >${{ minBet }}</span
            >
            <span class="text-[6px] text-gray-500 font-bold font-mono"
              >${{ maxBet }}</span
        ...
        <div id="quick-bet-group" class="flex gap-1">
          <button
            ...
            class="px-2 py-1 bg-white/5 border border-white/10 rounded-md text-[8px] font-bold text-gray-500 hover:text-white hover:bg-white/10 transition-all uppercase tracking-tight"
          >
            1/2
          </button>
          <button
          ...
          <button
            ...
            class="px-2 py-1 bg-white/5 border border-red-900/30 rounded-md text-[8px] font-bold text-red-400 hover:bg-red-500/10 transition-all uppercase tracking-tight"
          >
            ALL
          </button>
        </div>
      ...
      <div id="hud-actions-group" class="flex gap-1 md:gap-2 items-center">
        ...
        <template v-else-if="isMyTurn">
          <button
            ...
            class="px-5 py-2 rounded-xl border border-red-500/30 text-red-500 text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center hover:bg-red-500/20"
          >
            Fold
          </button>
          <button
            ...
            class="px-5 py-2 rounded-xl border border-white/10 text-gray-300 text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center hover:bg-white/10"
          >
            Check
          </button>
          <button
            ...
            class="px-5 py-2 rounded-xl border border-blue-500/30 text-blue-400 text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center hover:bg-blue-600/20"
          >
            Call
          </button>
