  ...
  >
    ...
    >
      ...
      <div class="absolute inset-0 pointer-events-none">
        <div class="absolute inset-0 bg-[radial-gradient(ellipse_80%_60%_at_50%_0%,rgba(212,168,83,0.07)_0%,transparent_60%)]"></div>
        <div class="absolute inset-0 bg-[radial-gradient(ellipse_60%_80%_at_50%_100%,rgba(13,31,15,0.9)_0%,transparent_70%)]"></div>
        <div class="absolute inset-0 bg-[repeating-linear-gradient(45deg,transparent,transparent_2px,rgba(255,255,255,0.012)_2px,rgba(255,255,255,0.012)_4px)]"></div>
      </div>
      ...
      >
        ...
        <div class="title-block text-center mb-8 animate-fade-up animation-delay-300">
          <p class="title-eyebrow font-mono text-[11px] tracking-[4px] uppercase text-[#D4A853] mb-2">Deush Poker · Torneo Finalizado</p>
          <h1 class="title-main font-bebas text-7xl sm:text-8xl leading-[0.95] tracking-[2px] bg-gradient-to-br from-[#F5D78E] via-[#D4A853] to-[#8A6A2A] bg-clip-text text-transparent">CAMPEÓN</h1>
          <p class="title-sub font-bebas text-3xl sm:text-4xl tracking-[6px] text-[#F5F0E8] opacity-85 mt-1">DEL TORNEO</p>
        </div>

        <!-- Winner Card -->
        <div class="winner-card w-full bg-gradient-to-br from-[#1E3D20] to-[#142A16] border border-[#D4A853]/30 rounded-2xl p-7 sm:p-8 mb-5 relative overflow-hidden animate-fade-up animation-delay-500">
          <div class="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#D4A853] to-transparent"></div>
          <div class="absolute -top-5 -right-2 text-[120px] text-black/25 rotate-[-15deg] select-none">♠</div>
          
          <p class="winner-label font-mono text-[10px] tracking-[3px] uppercase text-[#D4A853] mb-1.5">Ganador</p>
          <h2 class="winner-name font-bebas text-5xl sm:text-6xl tracking-[2px] leading-none text-[#F5F0E8] mb-4 relative z-10">{{ winnerName.toUpperCase() }}</h2>
          
          <div class="winner-stats flex gap-6 flex-wrap relative z-10">
            <div class="stat flex flex-col gap-0.5">
              <span class="stat-label font-mono text-[9px] tracking-[2px] uppercase text-[#9E9080]">Fichas Finales</span>
              <span class="stat-value font-mono text-base text-[#F5D78E] font-medium">{{ totalAmount.toLocaleString() }} chips</span>
            </div>
            <div class="stat flex flex-col gap-0.5">
              <span class="stat-label font-mono text-[9px] tracking-[2px] uppercase text-[#9E9080]">Torneo ID</span>
              <span class="stat-value font-mono text-base text-[#F5D78E] font-medium">{{ (certificate?.torneoId || pokerStore.torneoId || '').slice(0, 8) }}...</span>
            </div>
            <div class="stat flex flex-col gap-0.5">
              <span class="stat-label font-mono text-[9px] tracking-[2px] uppercase text-[#9E9080]">Fecha</span>
              <span class="stat-value font-mono text-base text-[#F5D78E] font-medium">{{ formattedDate }}</span>
            </div>
          </div>
        </div>

        <!-- Certificate Box -->
        <div class="cert-box w-full bg-[#060E07] border border-[#D4A853]/20 rounded-2xl p-7 sm:p-8 mb-5 relative animate-fade-up animation-delay-700">
          <div class="cert-header flex items-center justify-between mb-5">
            <span class="cert-title font-mono text-[10px] tracking-[3px] uppercase text-[#D4A853]">🎖 Código de Certificado</span>
            <div class="cert-badge flex items-center gap-1.5 px-2.5 py-1 bg-[#D4A853]/10 border border-[#D4A853]/25 rounded-full">
              <div class="cert-badge-dot w-1.5 h-1.5 bg-[#D4A853] rounded-full animate-blink"></div>
              <span class="cert-badge-text font-mono text-[9px] tracking-[1px] text-[#D4A853]">OFICIAL</span>
            </div>
          </div>
          ...
          <p class="code-info text-xs text-[#9E9080] leading-relaxed mb-6">
            Este código de <strong class="text-[#F5F0E8] font-medium">4 dígitos</strong> prueba que fuiste el ganador de este torneo.
            Guárdalo — nadie más lo tiene. Puedes verificarlo en la página oficial.
          </p>

          <div class="torneo-row flex items-center justify-between p-3 px-4 bg-white/[0.03] border border-white/[0.06] rounded-lg">
            <span class="torneo-id-label font-mono text-[10px] tracking-[2px] uppercase text-[#9E9080]">Torneo ID</span>
            <span class="torneo-id-value font-mono text-[12px] text-[#F5F0E8] mx-2 truncate">{{ certificate?.torneoId || pokerStore.torneoId }}</span>
            <button 
              @click="copyTorneoId" 
        ...
        <div class="verify-link w-full animate-fade-up animation-delay-900">
          <router-link 
            :to="{ name: 'verify', params: { torneoId: certificate?.torneoId || pokerStore.torneoId, code: certificate?.code } }"
            target="_blank"
        ...
        <div class="footer mt-8 text-center font-mono text-[10px] tracking-[2px] text-[#8A6A2A] opacity-60 animate-fade-up animation-delay-1100">
...
const formattedDate = computed(() => {
  const date = certificate.value?.date ? new Date(certificate.value.date) : new Date()
...
const copyTorneoId = () => {
  ...
  navigator.clipboard.writeText(id).then(() => {
    copyStatus.value = '✓ OK'
    setTimeout(() => { copyStatus.value = 'COPIAR' }, 1500)
  })
}
