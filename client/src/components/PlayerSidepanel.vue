      ...
      >
        ...
        <div :id="'sidepanel-player-avatar-box-' + player.id" class="relative">
          <div 
            ...
            :class="player.id === delayedActivePlayerId ? 'bg-yellow-500 border-yellow-400 text-black' : 'bg-gray-800 border-white/10 text-gray-400'"
          >
        ...
        <div :id="'sidepanel-player-info-' + player.id" class="flex-1 flex flex-col min-w-0">
          ...
          <div :id="'sidepanel-player-header-' + player.id" class="flex items-start justify-between gap-2 mb-1">
            <div class="flex flex-col min-w-0">
              <div class="flex items-center gap-2">
                <span 
                  ...
                  :class="{ 'text-yellow-400': player.id === delayedActivePlayerId }"
                >
                  {{ player.name }}
                </span>
                <!-- Connection Status -->
            ...
            <div class="flex flex-col items-end shrink-0">
              <span class="text-[10px] font-black text-gray-500 uppercase tracking-tighter leading-none mb-1">Stack</span>
              <div class="flex items-center gap-0.5">
                <span class="text-xs text-yellow-500 font-mono font-bold">$</span>
                <span :id="'sidepanel-player-chips-' + player.id" class="text-lg font-mono font-black text-white leading-none">{{ player.chips }}</span>
              </div>
            </div>
          </div>
          
          <!-- Bottom Row: Action & Bet -->
          <div :id="'sidepanel-player-bottom-row-' + player.id" class="flex items-end justify-between mt-2 pt-2 border-t border-white/5">
            <!-- Current Bet (Bottom Left) -->
            <div class="flex flex-col">
              <template v-if="player.currentBet > 0">
                <span class="text-[9px] font-black text-emerald-500/70 uppercase tracking-wider mb-0.5">Live Bet</span>
                <span :id="'sidepanel-player-card-bet-' + player.id" class="text-xl font-mono font-black text-emerald-400 leading-none transition-all duration-500">${{ player.currentBet }}</span>
              </template>
            </div>
            ...
            >
               ...
               <div :id="'sidepanel-player-action-value-wrapper-' + player.id" class="flex justify-end">
...
watch(() => props.activePlayerId, (newId, oldId) => {
