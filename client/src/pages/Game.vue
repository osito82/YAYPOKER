...
const props = defineProps({ isGuest: { type: Boolean, default: false } })
...
if (props.isGuest) playerName.value = `Spectator_${Math.floor(Math.random() * 1000)}`

watch(() => pokerStore.players, (newPlayers) => {
    if (!props.isGuest && secretCode) {
      const me = newPlayers.find((p) => p.secretCode === secretCode)
      if (me && me.name !== playerName.value) playerName.value = me.name
    }
  }, { immediate: true, deep: true })

const connectionOptions = { gameCode, playerName: playerName.value, secretCode: props.isGuest ? 'spectator' : secretCode, role: props.isGuest ? 'guest' : 'player' }
const urls = urlsFactory()
const { connectSocket, disconnectSocket, sendMessage } = useWebSocket(urls.server, connectionOptions)
...
const blindInfo = computed(() => {
  ...
  const amount = pokerStore.myInfo.requiredBlind || (pokerStore.getDisplayMsg?.toLowerCase().includes('small') ? pokerStore.smallBlind : pokerStore.bigBlind)
  return { amount, type: pokerStore.getDisplayMsg?.toLowerCase().includes('small') ? 'Small' : 'Big' }
})

const maxBet = computed(() => (myPlayer.value?.chips || 0) + (myPlayer.value?.currentBet || 0))
const minBet = computed(() => Math.min(pokerStore.getCurrentHighestBet + 20, maxBet.value))
const betAmount = ref(minBet.value)

const setQuickBet = (m) => {
  if (m === 'all') betAmount.value = maxBet.value
  else {
    const callAmount = Math.max(0, pokerStore.getCurrentHighestBet - (myPlayer.value?.currentBet || 0))
    let a = pokerStore.getCurrentHighestBet + (pokerStore.getPot + callAmount) * m
    betAmount.value = Math.min(Math.max(minBet.value, Math.round(a)), maxBet.value)
  }
}

// ✅ CORREGIDO: Ahora startGame recibe los datos (incluyendo bots) y los envía por WebSocket
const startGame = (data = {}) => {
  if (isConnected.value) {
    console.log('[DEBUG] SENDING startGame with bots:', data.bots);
...
const sendAction = (action) => {
  ...
  switch (action) {
    ...
    case 'bet': case 'raise': sendMessage({ action: action === 'bet' ? 'setBet' : 'setRise', [action === 'bet' ? 'chipsToBet' : 'chipsToRiseBet']: betAmount.value }); break
    case 'blind': sendMessage({ action: 'setBet', chipsToBet: blindInfo.value.amount }); break
  }
}
...
onMounted(() => {
  ...
  timeInterval = setInterval(() => { serverTime.value = new Date().toLocaleTimeString() }, 1000)
})
