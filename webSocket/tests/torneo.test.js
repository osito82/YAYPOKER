...
describe('Torneo Class', () => {
  ...
  it('should remove inactive matches', () => {
    const now = Date.now()
    const activeMatch = { gameId: 'G1', lastActivity: now, players: [{ name: 'P1' }] }
    const inactiveMatch = { gameId: 'G2', lastActivity: now - 4000000, players: [{ name: 'P2' }] } // > 1 hour
    const abandonedMatch = { gameId: 'G3', lastActivity: now - 700000, players: [] } // > 10 min and no players
